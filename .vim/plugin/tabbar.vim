...
noremap <unique> <script> <Plug>tbstart  :call <SID>Tb_Start(0, -1)<CR>:<BS>
noremap <unique> <script> <Plug>tbstop   :call <SID>Tb_Stop(0)<CR>:<BS>
noremap <unique> <script> <Plug>tbaut    :call <SID>Tb_Aup(-1)<CR>:<BS>
noremap <unique> <script> <Plug>tbtoggle :call <SID>Tb_Toggle()<CR>:<BS>
" %%
" TabBar commands ~~
if !exists(':TbStart')
      command! TbStart  call <SID>Tb_Start(0, -1)
endif

if !exists(':TbStop')
      command! TbStop  call <SID>Tb_Stop(0)
endif
...
if !exists(':Tbbn')
      command! Tbbn call <SID>Bf_Cycle(0)
endif
if !exists(':Tbp')
...
function! <SID>Bf_CrSel()
  ...
  " Make sure we are in our window
  if bufname('%') != '-TabBar-' && bufname('%') != '__Tag_List__'
    if g:Tb_DBG_LVL > 0
